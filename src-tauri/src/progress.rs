//! 章节进度相关纯函数 —— 解析 / 归一 / 格式化 / 累加。
//!
//! 规则与 `src/shared/progress.ts` 完全一致,行为对齐(见 vitest 单测)。
...
/// - `total=0`/负/非数 → fallback 为 `total=None`(保留 current,UI 显示"连载中")
...
/// 把用户/表单输入的 `Progress | null | undefined` 规整为 `Progress | null`。
...
/// 格式化进度为可读字符串。空 / 全 0 / null → ""。
pub fn format_progress(p: Option<&Progress>) -> String {
    let p = match p {
        Some(p) => p,
...
mod tests {
    ...
    fn parse_progress_null_or_undefined() {
        ...
        // undefined 在 JSON 里不存在,传 null 等价
    }
    ...
    #[test]
    fn format_progress_total_known() {
        assert_eq!(
    ...
    #[test]
    fn format_progress_total_null_with_current() {
        assert_eq!(
            format_progress(Some(&Progress {
                current: 12,
                total: None
    ...
    #[test]
    fn format_progress_total_null_no_current() {
        assert_eq!(
            format_progress(Some(&Progress {
                current: 0,
                total: None
