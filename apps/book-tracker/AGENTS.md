...
43. **v2025-09 StampRow wikilink `[[` 不触发 + 无法修改时间戳时间**(修复):**现象**:用户报告 (a) 在 stamp 的 note textarea 里逐字符输入 `[[` 不弹角色选择 picker;(b) StampRow 的时间戳只显示,没法编辑。**根因**:
    - (a) StampRow 之前直接 `value={stamp.note}` + 每次 keystroke 同步调 `onEditNote → handleEditNote → sortStamps → onChange prop → onSetStamps → setEpisodeStamps IPC(异步)`。React 18 controlled input 在快速连续 keystroke 时,prop 还没 commit(IPC 异步进行中),DOM value 与 prop value 不同步,React 会强制重置 DOM 到 prop —— **第二次 keystroke 的字符被吞掉**,只触发 1 次 onChange,`[[` 检测窗口错过。EpisodeEditor / CharacterEditor 的 wikilink 一直 work,正是因为它们用**本地 noteDraft + debounce 500ms flush**(state 由 React useState 同步管理,DOM 不会反向重置)。
    - (b) StampList 内**定义了 `handleEditStart` / `handleEditEnd` 函数但 UI 完全没接** —— 是死代码。StampRow 时间戳只渲染 `<span class="stamp-row-time">{formatStamp(stamp.start)}</span>`,没有 input。
...
45.5 **v2025-09 / 2026-09 中文输入法「汉字突然变成拼音」**(修复):**现象**:用户在时间戳笔记 / 角色笔记 / 集笔记 / 主笔记 4 处 wikilink textarea 任何一处用中文输入法打字,每敲一个字母正常,但**按空格选词的瞬间,刚选中的汉字被反向覆盖成拼音字符**(看起来像"输入框里汉字只剩拼音字符串")。**根因**:`useWikilinkTextarea.handleChange` 完全没处理 IME composition 事件 —— 拼音期间每敲一个字母 React 都收到 onChange,e.target.value 是拼音字符(如 "ni"),hook 直接 `setValue(newValue)`,React 18 controlled input 用拼音 prop value 同步渲染;**用户按空格选词「你」时,IME 想把 DOM 里的拼音替换为汉字,但 React 在 commit 后用 prop value (=拼音) 覆盖 DOM**,于是 textarea 显示的仍是拼音,不是汉字。**修复**(2026-09):在 hook 内挂 IME compositionstart / compositionend **DOM 监听器**(用 `ta.addEventListener` 而非 React 的 `onCompositionStart`/`onCompositionEnd` prop,后者跟 React 18 controlled input 同样有时序问题),加 `isComposingRef` 跟踪状态,`handleChange` 第一行判断 `if (isComposingRef.current || (e.nativeEvent as InputEvent).isComposing) return` —— composition 期间跳过 setValue,让 IME 自己管 DOM;`compositionend` 监听器手动调 `setValueRef.current(ta.value)`(已是汉字)同步给 React state。**关键点**:用 `setValueRef` 缓存父组件传入的 setValue(父组件每次 render 通常传新箭头函数,直接进 effect 依赖会导致无限重挂)。**判定**:任何 controlled input(不仅是 wikilink textarea,所有 textarea / input)都必须正确处理 IME composition —— 「本地 draft + debounce flush」(§十.43)解决了**keystroke 丢失**,但**不解决 IME 选词覆盖**;两者是独立 bug。**回归测试**:`src/renderer/components/__tests__/useWikilinkTextarea.ime.test.tsx` 3 个 case 覆盖(拼音期间 setValue 不被调 / compositionend 同步汉字 / 结束后正常打字)。**注意**:jsdom 不实现真实 IME,测试用 `ta.dispatchEvent(new CompositionEvent('compositionstart', {bubbles: true}))` + `Object.defineProperty(ta, 'value', {value: '你', configurable: true})` 模拟序列(AGENTS.md §十.44 的「不依赖 commit 时序」原则同样适用 —— 多次连续 `fireEvent.change` 时 React state 异步未 commit 会让 controlled input 反向重置 DOM,导致 setValue 收到的值与预期不符)。**适用范围**:此修复在 hook 内部统一处理,**一处修 5 处生效**(Book.notes / Character.notes / EpisodeRecord.note / TimeStamp.note + 后续若有新 wikilink textarea 接入零成本)。
