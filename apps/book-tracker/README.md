...
- 添加一部作品：作品名 / 作品类型（书、动画、电视剧、电影、其他）/ 作者·主创 / 国家地区 / 年份 / 译者
  - **类型感知字段标签**：按 `WorkKind` 自动切换"作者 / 原作·主创 / 导演"、"出版 / 开始 / 首播 / 上映年份"、"原产国 / 制片国家"；译者字段仅书显示
- 设置面板：新建作品默认类型 + 展示筛选（全部 / 按类型，按钮式高亮选择）
- 给一部作品加前置依赖，多对多网状结构
- 解锁规则：全部完成 (`all`) / 阈值型 (`any_of` N 部) / **二选一组合**（如 "A 或 B + C 必须"）
- 日常模式：自动列出"现在能看的作品"，按反向度数排序（基础作品靠前）
- 编辑模式：增删改、状态切换（想看 / 搁置 / 在读 / 在看 / 已读 / 弃读）
  - **`watching`（在看）**仅对非电影类型可选 —— 与 `reading` 语义一致,色族同属「进行中」;解决"已看完后再追一遍时 `reading` 措辞尴尬"
...
- **作品排名**（两两对比 Elo）：给"已读"作品做全序排名，TopBar「排」按钮或快捷键 `r`
  ...
  - 排名列表视图：按 Elo 评分倒序，条形图 + 标题 + 作者 + 对比次数 + 评分
  - 对比视图：左右两本候选（标题 + 作者 + 年份 + 国家 + tags），点击选 winner，支持「跳过」「平局」
  - 池 = `status === 'finished'` 且 `kind === 选中 kind` 的作品
- 数据：每部作品一个 Markdown 文件 + JSON 关系图 + JSON 排名历史，可直接 `git init` 管理
...
`tags` 是字符串数组,空数组 `[]` 也写盘（保留"清空"语义）;`notes` 空串**不写盘**
（避免污染 frontmatter）。
...
只有 `finished` 才算"已掌握"。`reading` / `watching` 语义一致 —— `watching` 仅表单对
非电影类型暴露,Rust 端接受任意类型的 `watching` 序列化(保留扩展空间)。
