...
// - 展开区:标题输入 / watched toggle / 笔记 textarea / 删除按钮
// - 顶部操作栏:已看统计 / +1 / -1 / 清空
//
// 状态:
...
// - 所有变更走 store action(setEpisode* / episodeBump / clearEpisodes / setSeasons)

import { useEffect, useMemo, useRef, useState } from 'react'
import { useBooksStore } from '../store/books'
import { useEpisodeStats, useEpisodesForBook, useSeasonsForBook } from '../store/selectors'
import { episodeKey, parseEpisodeKey } from '@shared/types'
    ...
    <div className="episode-editor">
      ...
      <div className="episode-editor-foot">
        <button
          ...
          onClick={() => {
            if (!confirm(`删除 S${pad2(season)}E${pad2(episode)} 的所有记录(watched / 笔记 / 标题)?`)) return
            // 三步清零:watched=false → 空 note → 空 title(后两步空串会触发 service 删 key)
            onSetWatched(false)
            onSetNote('')
            onSetTitle('')
            onClose()
