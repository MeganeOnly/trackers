...
/**
 * 关系图右侧的卡片容器：跟随全局 selectedId 自动渲染对应目标的详情。
 * - 已选中 → 渲染 GoalCard（内联可编辑）
 * - 未选中 → 显示提示（让用户知道怎么用）
 */
export function GoalCardContainer({ onClose }: GoalCardContainerProps): JSX.Element {
  ...
  if (!selectedId || !goalExists) {
    return (
      <div className="goal-card-empty muted">
        <p>← 点左侧图中的节点查看这个目标的详情</p>
        <p className="hint">所有操作（状态 / 进度 / 前置 / 字段编辑）都在这里</p>
      </div>
    )
  }

  return <GoalCard goalId={selectedId} onClose={onClose} />
}
